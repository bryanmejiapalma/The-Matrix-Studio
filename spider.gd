...
# Grab a reference to your AnimatedSprite2D node
...
func _ready():
	...
	# Start playing your walk animation immediately when spawning
	if animated_sprite:
...
func _physics_process(_delta):
	...
	if is_instance_valid(player) and not player.is_dead:
		...
		if player.has_method("is_hidden") and player.is_hidden:
			...
			# Keep playing the walk animation while moving
			if animated_sprite and velocity.length() > 0:
				animated_sprite.play("walk")
...
func take_damage(amount):
	...
	if health <= 0:
		print("Spider died!")
		spawn_loot()
		queue_free() # This removes the spider from the game when it dies
